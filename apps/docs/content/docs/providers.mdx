...
For ZCode, the model list is read from `~/.zcode/cli/config.json` (the `provider` catalog); picking an entry sends that `provider/model` selector to the runtime's session. Leaving the model unset follows the config's `model.main`. An empty list is expected when the config is missing and is not a sign that the runtime is offline.
